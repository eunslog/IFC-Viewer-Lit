...
export default function measurement(components: OBC.Components, world: OBC.World) {
  ...
  const setupEvents = () => {
    ...
    window.removeEventListener("dblclick", createDimenison);
    window.removeEventListener("keydown", deleteDimension);

    if (enabled && selected !== "Volume") {
      window.addEventListener("dblclick", createDimenison);
  ...
  return BUI.Component.create<BUI.PanelSection>(() => {
    return BUI.html`
      <bim-panel-section label="Measurements" style="pointer-events: auto;" collapsed>
        <div style="display: flex">
          <bim-checkbox id="measurement-checkbox" @change="${onEnabled}" label="Enabled"></bim-checkbox>
          <bim-button @click="${deleteAll}" label="Delete all"></bim-button>
            ${dropDown}
        </div>
      </bim-panel-section>
    `;
